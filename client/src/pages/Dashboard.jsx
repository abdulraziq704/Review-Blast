...
const Dashboard = () => {
    ...
    useEffect(() => {
        const initialization = async () => {
            try {
                // 1. Fetch Fresh User Data from DB (The source of truth)
                // We do this first to ensure contactLimit and other user fields are fresh
                ...
                const { data: statsData } = await api.get('/contacts/stats');
                setStats({
                    totalContacts: statsData.totalContacts || 0,
                    sentMessages: statsData.sentHistory || 0
                });
            } catch (err) {
    ...
    // Animate bar from 0% to actual width after stats load
    useEffect(() => {
        if (!statsLoading) {
            const targetWidth = Math.min(
                (stats.sentMessages / (user?.contactLimit || 1)) * 100,
                100
            );
            // Small delay so transition is visible from 0
            const timer = setTimeout(() => setAnimatedWidth(targetWidth), 80);
            return () => clearTimeout(timer);
        } else {
            setAnimatedWidth(0);
        }
    }, [statsLoading, stats.sentMessages, user?.contactLimit]);
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-50">
                                ...
                                <div className="bg-gray-100 h-4 w-full rounded-full overflow-hidden shadow-inner">
                                    <div
                                        ...
                                        style={{ width: statsLoading ? '0%' : `${animatedWidth}%` }}
                                    ></div>
                                </div>
                                <div className="flex justify-between mt-2">
                                    <span className="text-sm font-medium text-gray-600">
                                        {statsLoading ? 'Calculating capacity...' : `${stats.sentMessages} / ${user.contactLimit || 0} Messages sent`}
                                    </span>
                                    <span className="text-sm font-bold text-indigo-600">
                                        {statsLoading ? '...' : `${Math.round((stats.sentMessages / (user.contactLimit || 1)) * 100)}%`}
                                    </span>
                                </div>
                            </div>
                        </div>
